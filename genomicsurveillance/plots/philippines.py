...
from genomicsurveillance.data import get_ph
...
def plot_lad_ph(
    ...
    lineage_tensor,
    lin_date_idx,
    start_date,  # example: "2020-09-01"
    show_start_date,  # example: "2020-09-01"
    lin_idx, # list of indices corresponding to lineages in lineage_tensor
    colors,  # list of colors (may be hexadecimals)
    ...
    ax[0].set_title(ph.Name.values[lad])
    for i, l in enumerate(lin_idx):
    ...
    secax = ax[0].secondary_yaxis(
        "right",
        functions=(
            lambda x: x / ph.population.values[lad] * 1e5,
            lambda x: x / 1e5 * ph.population.values[lad],
        ),
    )
    ...
    for i, l in enumerate(lin_idx):
        ...
        ax[1].scatter(
            ...
            c=f"C{lin_idx%10}" if colors is None else colors[i],
